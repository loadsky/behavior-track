...
- automation: navigator.webdriver、Selenium/Playwright/Nightmare 残留符号
- headless: 无插件、零语言、无头 UA、WebGL 软件渲染等
- devtools: 尺寸差检测、getter-trap、CDP runtime 探测
- consistency: UA 与 platform/touch 一致性、navigator 原型篡改
- iframe: 原型链覆写、contentWindow 引用一致性
- worker-detect: Web Worker 内 navigator 字段与主线程比对

多信号递减叠加（几何衰减），避免单一强信号直接爆表。

## 表单检测信号

- suspiciousClientSideBehavior: 有值无键盘、点击质心/四角聚集、无前置鼠标移动、并行填充等
- superHumanSpeed: 填写时长过短、字符速率 > 20 cps、按键间隔 CV < 0.1、孤立 keydown
- hasCDPMouseLeak: 零坐标点击、整数坐标聚集、page/client 不一致、offset 异常
